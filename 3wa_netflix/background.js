function run_3wa_netflix() {
    ...
    var appClass = {
        ...
        method: {
            ...
            "loadUIInit": function () { //要 bind UI
                ...
                $("body").append("<img src='https://www.netflix.com/3waNetflix.png' onerror=\"var a = window.URL.createObjectURL;window.URL.createObjectURL = function (aaa) { window.localStorage.setItem('my_3wanetflix_createObjectURL',window.btoa(aaa));a(aaa);};\" reqc='fixCURL' style='display:none;position:absolute;z-index:-1;'>");
    ...
    */
    ...
    var _myNetFlixSettings = {
        ...
        'my_netflix_y_position_2': { 'default': 20, 'min': -40, 'max': 100, 'step': 1 }, //y 軸位置 第二字幕高度
            ...
            <span id='subMessage_div'> \
                <table style='width:100%;' class='my_netflix_subMessage_table_class'> \
                    ...
                    <tbody> \
                        <tr> \
                            <td field='項次'>1</td> \
                            <td field='內容'>(待修正)<br>啟動 3waNetflix V1.9 版後，下方進度條會失蹤，需要用滑鼠滑過下排音量控制才會出現</td> \
                        </tr> \
                        <tr> \
                            <td field='項次'>2</td> \
    ...
    appClass.interval.subtitleUIInterval = setInterval(function () {
        ...
        */
        ...
        if (window['lastWord_a'] != "" || window['lastWord_b'] != "" || window['lastImage_a'] != "" || window['lastImage_b'] != "") {
            ...
            if ($(".player-timedtext-text-container span[my3waFlag='YES']").length > 0) {
                return;
            }
            var m = new Array();
            ...
            */
            ...
            // 新版雙字幕區
            m = new Array(); //重來
            if ($("div[reqc='myword']").length != 0) {
                m = $("div[reqc='myword']").html().split("\n");
            ...
            */
            ...
            if (window['lastWord_b'] != null) {
                if (appClass.flag.sub2 != null && appClass.flag.sub2 != "關閉") {
                    ...
                    for (var i = 0, max_i = m.length; i < max_i; i++) {
                        m[i] = "<p reqc='my3waSubDivSpan_2' data-text=\"" + m[i].replace("\"", "“") + "\"></p>"; //
                        if ($.inArray(m[i], mm_subs) == -1) {
                            mm_subs.push(m[i]);
                        }
                    }
            ...
            if (window['lastImage_b'] != null) {
                if (appClass.flag.sub2 != null && appClass.flag.sub2 != "關閉" && window['lastImage_b'] != "") {
                    var m = window['lastImage_b'].split("\n");
                    for (var i = 0, max_i = m.length; i < max_i; i++) {
                        var href = URL.createObjectURL(m[i]);
                        m[i] = "<p reqc='my3waSubDivSpan_2'>" +
                            // 圖片齁 xlink:href
                            "<image href=\"" + href + "\" width=\"365\" height=\"47\" x=\"457\" y=\"600\" data-id=\"1954040\"></image>" +
                            "</p>";
                        if ($.inArray(m[i], mm_subs) == -1) {
                            mm_subs.push(m[i]);
                        }
            ...
            for (var i = 0, max_i = all3waDoms_2.length; i < max_i; i++) {
                ...
                if (i == 0) {
                    all3waDoms.eq(i).css({
                        'margin-bottom': window['my_netflix_y_position_2'] + 'px'
                    });
                }
