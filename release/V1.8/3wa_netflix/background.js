    ...
    var appClass = {
        ...
        method: {
            ...
            "smallComment": function (message, seconds, is_need_motion, cssOptions) {
                //畫面的1/15	
                if (this.$("#mysmallComment").length == 0) {
                    this.$("body").append("<div id='mysmallComment'><span class='' id='mysmallCommentContent'></span></div>");
                    this.$("#mysmallComment").css({
                        'display': 'none',
                        'position': 'fixed',
                        'left': '0px',
                        'right': '0px',
                    ...
                    this.$("#mysmallCommentContent").css({
                        'color': '#fff',
                    ...
                    this.$("#mysmallCommentContent").css(cssOptions);
                }
                this.$("#mysmallCommentContent").html(message);
                if (is_need_motion == true) {
                    ...
                    //$("#mysmallComment").stop();
                    this.$("#mysmallComment")[0].style.display = "inline";
                    clearTimeout(window['smallComment_TIMEOUT']);
                    window['smallComment_TIMEOUT'] = setTimeout(function () {
                        this.$("#mysmallComment")[0].style.display = "none";
                    }.bind(this), seconds);
                }
            }, //浮動說明
            "isImageFont": function () {
                return (this.$(".image-based-subtitles svg image").length > 0);
            },
...
<style reqc='s'>   
  ...
  .image-based-subtitles svg image{
    ...
    window['wtf'] = setInterval(function () {
        ...
        if (appClass.method.$(".image-based-subtitles svg image").length > 0) { //2022/03/18以前的舊版，圖片型字幕            
            //2022/03/18以前的舊版
            if (appClass.method.$(".image-based-subtitles svg image[my3waFlag='YES']").length > 0) return;
            var dom_image = appClass.method.$(".image-based-subtitles svg image");
            if (dom_image.attr("orin_height") == null) {
                dom_image.attr("orin_height", appClass.method.$(".image-based-subtitles svg image").attr("height"));
                dom_image.attr("orin_x", appClass.method.$(".image-based-subtitles svg image").attr("x"));
                dom_image.attr("orin_y", appClass.method.$(".image-based-subtitles svg image").attr("y"));
            }
        ...
        else if (appClass.method.$(".player-timedtext-text-container span").length > 0) { //2022/03/21 新版 0.7 文字型字幕
            //2022/03/21 新版 0.7
        ...
        `);
            ...
            var all3waDoms = appClass.method.$("div[reqc='my3waSubDiv'] *");
            for (var i = 0, max_i = all3waDoms.length; i < max_i; i++) {
                all3waDoms[i].style.setProperty("letter-spacing", window['my_netflix_fontspace'] + 'px', "important");            
            }
